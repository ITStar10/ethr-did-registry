...
describe('ERC1056', () => {
  ...
  before(async () => {
    ...
    // identity = accountList[0]
    // identity2 = accountList[1]
    // delegate = accountList[2]
    // delegate2 = accountList[3]
    // delegate3 = accountList[4]
    // badBoy = accountList[5]
  ...
  describe('bulkAdd()', () => {
    ...
    const delegateParams: { identity: string; delegateType: BytesLike; delegate: string; validity: BigNumberish }[] = []
    const attributeParams: { identity: string; name: BytesLike; value: BytesLike; validity: BigNumberish }[] = []
    before(async () => {
      delegateParams.push({
      ...
      tx = await didReg.connect(identity).bulkAdd(delegateParams, attributeParams)
